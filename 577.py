...
while True:
    if option==1:
        ...
        filepath=input("enter filepath: ")
        command=f"grep -i 'failed password for invalid user' {filepath}  | awk '{{ match($0, /[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/); ip = substr($0, RSTART, RLENGTH); print ip }}' | sort | uniq -c | sort -nr"
        result=subprocess.run(command, shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        output=result.stdout.decode()
        print(output)
        break
        ...
        command=f"grep -i 'Invalid user' auth.log | awk '{{print $(NF-5)}}' | sort | uniq -c | grep -v -i 'invalid'"
        result=subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output=result.stdout.decode()
        print(output)
        break
